...
ALTER PROCEDURE dbo.usp_Shopping_Order_GetPagging
...
AS
	...
	IF LEN(@SortColumn) = 0
		SELECT @query += ' ORDER BY ProductName ' + IIF(LEN(@SortDir) = 0, 'ASC', @SortDir)
	ELSE
		SELECT @query += ' ORDER BY ' + @SortColumn + ' ' + IIF(LEN(@SortDir) = 0, 'ASC', @SortDir)
